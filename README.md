...
uses the development credentials `sentra`/`sentra`, so do not use it in production. The schema is created and updated at
startup by
...
with the Instagram username and the API password of a monitored account, and works on that account only. The Instagram
password is never accepted by the API. Lists reflect the last sync.
